...
public:
    ...
    virtual std::unique_ptr<HalWrapper> connectEvervolv();
    virtual void reset();
};
...
public:
    ...
    virtual HalResult<int> getFeature(vendor::evervolv::power::Feature) override;

private:
    std::mutex mConnectedHalMutex;
    std::mutex mConnectedEvervolvHalMutex;
    ...
    std::shared_ptr<HalWrapper> mConnectedEvervolvHal GUARDED_BY(mConnectedEvervolvHalMutex) = nullptr;
    const std::shared_ptr<HalWrapper> mDefaultHal = std::make_shared<EmptyHalWrapper>();

    std::shared_ptr<HalWrapper> initHal();
    std::shared_ptr<HalWrapper> initEvervolvHal();
    template <typename T>
